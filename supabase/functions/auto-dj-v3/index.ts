...
function analyzeTrajectory(tracks: Track[]): Analysis {
  ...
  // Extract artists and build context
  const recentArtists = tracks.slice(0, 5).map((t) => t.artist);
  const allTitles = tracks.map((t) => t.title).join(" • ");
...
function analyzeStylePatterns(tracks: Track[]): {
  ...
  // Simplified style detection based on common patterns
...
function estimateEnergyTrend(tracks: Track[]): {
  ...
  // Heuristic: some genres tend toward higher energy
  ...
  const current = tracks.length > 0 ? Math.round(totalEnergy / tracks.length) : 50;

  // Determine trend from last 3 tracks
  let trend = "stable";
  if (tracks.length >= 2) {
